...
def OptimizeDensityConf(config, ions = None, rhoini = None):
    ...
    for key in sorted(energypotential.keys()) :
        ...
        if key == 'KE' :
            ene = 0.0
            for key1 in value :
                print('{:>10s} energy (eV): {:22.15E}'.format('KE-'+ key1, value[key1].energy * ENERGY_CONV['Hartree']['eV']))
    ...
    print('{:>10s} energy (eV): {:22.15E}'.format('TOTAL', energypotential['TOTAL'].energy * ENERGY_CONV['Hartree']['eV']))
    print('-' * 80)
