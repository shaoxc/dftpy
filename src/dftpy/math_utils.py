...
def LineSearchDcsrch(func,
    ...
    for i in range(maxiter):
        alpha1, func1, derfunc1, task = minpack2.dcsrch(
            alpha1, func1, derfunc1, c1, c2, xtol, task, amin, amax, isave,
            dsave)
        if task[:2] == b'FG':
            func1 = func(alpha1)
...
def LineSearchDcsrch2(func,alpha0 = None, func0=None, \
    ...
    for i in range(maxiter):
        alpha1, x1, g1, task = minpack2.dcsrch(alpha1, x1, g1, c1, c2, xtol,
...
def interpolation_3d(arr, nr_new, interp='map'):
    ...
    if interp == 'map':
        ...
        new_values = interpolate.griddata(points,
                                          values.ravel(), (X, Y, Z),
                                          method='linear')
    return new_values
...
def restriction(arr, scheme='bilinear'):
    ...
    f[1:nr[0] + 1, :, :] = 0.25 * (f[:nr[0], :, :] +
                                   2.0 * f[1:nr[0] + 1, :, :] + f[2:, :, :])
    f[:, 1:nr[0] + 1, :] = 0.25 * (f[:, :nr[0], :] +
                                   2.0 * f[:, 1:nr[0] + 1, :] + f[:, 2:, :])
    f[:, :, 1:nr[2] +
      1] = 0.25 * (f[:, :, :nr[2]] + 2.0 * f[:, :, 1:nr[2] + 1] + f[:, :, 2:])
    nr2 = nr // 2 + 1
...
def prolongation(arr, scheme='bilinear'):
    ...
    #line
    new_values[
        1::2, ::2, ::2] = f2[:nr[0], :nr[1], :nr[2]] + f2[1:, :nr[1], :nr[2]]
    new_values[::2,
               1::2, ::2] = f2[:nr[0], :nr[1], :nr[2]] + f2[:nr[0], 1:, :nr[2]]
    new_values[::2, ::2,
               1::2] = f2[:nr[0], :nr[1], :nr[2]] + f2[:nr[0], :nr[1], 1:]
    #plane
