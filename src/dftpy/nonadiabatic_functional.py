...
def NonadiabaticFunctional(density, calcType=["V"], **kwargs):
    ...
    v_p_kf_threehalves = 1.0/4.0 * np.pi ** 2 * omega ** 2 * (16.0 - np.pi **2) / q / q * rho_over_kf_threehalves_of_g
    q[0, 0, 0] = 0.0
    v_p_kf[0, 0, 0] = 0.0
    ...
    #else:
    #    e_h = 0
    v_p_of_r = v_p_kf.ifft(force_real = force_real) / kf + v_p_kf_square.ifft(force_real = force_real) / kf_square + v_p_kf_threehalves.ifft(force_real = force_real) / kf_threehalves# + v_p_kf_cube.ifft(force_real = force_real) / kf_cube)
