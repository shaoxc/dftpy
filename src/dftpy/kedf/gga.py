    ...
    if functional == "LKT":  # \cite{luo2018simple}
        ...
        if calcType != "Energy":
            ...
            dFds2[mask2] = (
                10.0 / 3.0
                - parms[0] ** 2
                + 5.0 / 6.0 * parms[0] ** 4 * s2[mask2]
                - 61.0 / 120.0 * parms[0] ** 6 * s2[mask2] ** 2
            )
            dFds2 /= tkf0 ** 2
        ...
        if not parms:
            parms = [0.0055, 0.0253, 0.072]
        bs = b * s
        bs2 = bs * bs
        F = (
            1.0
            + parms[0] * bs2 / (1.0 + parms[1] * bs * np.arcsinh(bs))
        ...
        if calcType != "Energy":
            ...
            dFds2 = (
                -(parms[0] * parms[1] * bs2) / np.sqrt(1 + bs2)
        ...
        if not parms:
            parms = [7.0 / 324.0 / (18.0 * np.pi ** 4) ** (1.0 / 3.0), 0.861504, 0.044286]
        bs = b * s
        bs2 = bs * bs
        ...
        if not parms:
            parms = [0.093907, 0.26608, 0.0809615, 100.0, 76.320, 0.57767e-4]  # (A1, A2, A3, A4, A, B1)
        ss = s / tkf0
        s2 = ss * ss
        s4 = s2 * s2
        ...
        if calcType != "Energy":
            ...
            dFds2 = 2.0 * (
                parms[1] + (parms[3] * s2 - 1) * parms[2] * np.exp(-parms[3] * s2) - 4.0 * parms[5] * s2
            ) / Fb - Fa_s2 * ss / (Fb * Fb) * (
                (parms[0] * parms[4] * ss) / (parms[4] ** 2 * s2 + 1)
        ...
        if not parms:
            parms = [0.19645, 0.2747, 0.1508, 100.0, 7.7956, 0.004]  # (A1, A2, A3, A4, A, B1)
        ss = s / tkf0
        s2 = ss * ss
        s4 = s2 * s2
        ...
        if not parms:
            parms = [1.0, 88.3960, 16.3683, 88.2108]
        ss = s / tkf0
        s2 = ss * ss
        s4 = s2 * s2
        Fa = parms[0] + parms[1] * s2 + parms[2] * s4
        ...
        if not parms:
            parms = [1.7107, -7.2333, 61.645, -93.683]
        ss = s / tkf0
        s2 = ss * ss
        s4 = s2 * s2
        s6 = s4 * s2
        ...
        if not parms:
            parms = [2.777028126, 2.777028126 - 40.0 / 27.0]
        ss = s / tkf0
        s2 = ss * ss
        s2[s2 < tol2] = tol2
        s4 = s2 * s2
