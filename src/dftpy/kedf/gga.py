...
def GGAStress(rho, functional='LKT', energy=None, potential=None, **kwargs):
    ...
    for i in range(3):
        dRho_ij.append(
            (1j * g[..., i][..., np.newaxis] * rhoG).ifft(force_real=True))
    stress = np.zeros((3, 3))
    ...
    for i in range(3):
        for j in range(i, 3):
            stress[i, j] = np.einsum('ijk, ijk', vkin2,
                                     dRho_ij[i] * dRho_ij[j])
            if i == j:
...
def GGAFs(s, functional='LKT', calcType='Both', parms=None, **kwargs):
    ...
    if functional == 'LKT':  # \cite{luo2018simple}
        ...
        F[mask2] = 1.0 + (
            5.0 / 3.0 -
            0.5 * parms[0]**2) * s2[mask2] + 5.0 / 24.0 * parms[0]**4 * s2[
                mask2]**2  #- 61.0/720.0 * parms[0] ** 6 * s2[mask2] ** 3
        if calcType != 'Energy':
            dFds2[mask] = 10.0 / 3.0 - parms[0] * np.sinh(
                parms[0] * ss[mask]) / np.cosh(
                    parms[0] * ss[mask])**2 / ss[mask]
            dFds2[mask1] = 10.0 / 3.0
            dFds2[mask2] = 10.0 / 3.0 - parms[0]**2 + 5.0 / 6.0 * parms[
                0]**4 * s2[mask2] - 61.0 / 120.0 * parms[0]**6 * s2[mask2]**2
            dFds2 /= (tkf0**2)

    elif functional == 'DK':  # \cite{garcia2007kinetic} (8)
        if not parms:
            parms = [0.95, 14.28111, -19.5762, -0.05, 9.99802, 2.96085]
        x = s * s / (72 * cTF)
        Fa = (9.0 * parms[5] * x**4 + parms[2] * x**3 + parms[1] * x**2 +
              parms[0] * x + 1.0)
        ...
        if calcType != 'Energy':
            dFds2 = 2.0 * parms[0] / Fb - (
                parms[0] * parms[1] * bs * np.arcsinh(bs) +
                Fa * parms[1] / np.sqrt(1.0 + bs2)) / (Fb * Fb)
            dFds2 *= (b * b)
        ...
        bs = b * s
        bs2 = bs * bs
        F = 1.0 + parms[0] * bs2 / (1.0 + parms[1] * bs * np.arcsinh(bs)
                                    ) - parms[2] * bs / (1.0 + 2**
                                                         (5.0 / 3.0) * bs)
        ...
        if calcType != 'Energy':
            ...
            dFds2[mask] -= parms[2] / (1.0 +
                                       2**(5.0 / 3.0) * bs[mask])**2 / bs[mask]
        ...
        if calcType != 'Energy':
            dFds2 = (2 * parms[0] *
                     (parms[1] * bs2 + 5.0)) / (5 * (1.0 + parms[1] * bs2) *
                                                Fb) * (b * b)

    elif functional == 'DK87':  # \cite{garcia2007kinetic} (15)
        if not parms:
            parms = [
                7.0 / 324.0 / (18.0 * np.pi**4)**(1.0 / 3.0), 0.861504,
                0.044286
            ]
        bs = b * s
        bs2 = bs * bs
        ...
        F = 1.0 + Fa / Fb
        if calcType != 'Energy':
            dFds2 = parms[0] * (2.0 + 3.0 * parms[1] * bs + parms[1] *
                                parms[2] * bs2 * bs) / (Fb * Fb) * (b * b)
        ...
        F = Fa**(1.0 / 15.0)
        if calcType != 'Energy':
            dFds2 = 2.0 / 15.0 * (parms[0] + 2 * parms[1] * s2 +
                                  3 * parms[2] * s4) / Fa**(14.0 / 15.0)
            dFds2 /= (tkf0 * tkf0)

    elif functional == 'PW91O':  # \cite{gotz2009performance} (20)
        if not parms:
            parms = [0.093907, 0.26608, 0.0809615, 100.0, 76.320,
                     0.57767E-4]  #(A1, A2, A3, A4, A, B1)
        ss = s / tkf0
        s2 = ss * ss
        s4 = s2 * s2
        Fa = (parms[1] -
              parms[2] * np.exp(-parms[3] * s2)) * s2 - parms[5] * s4
        Fb = 1.0 + parms[0] * ss * np.arcsinh(parms[4] * ss) + parms[5] * s4
        F = 1.0 + Fa / Fb
        if calcType != 'Energy':
            Fa_s2 = (parms[1] -
                     parms[2] * np.exp(-parms[3] * s2)) - parms[5] * s2
        ...
        if not parms:
            parms = [0.19645, 0.2747, 0.1508, 100.0, 7.7956,
                     0.004]  #(A1, A2, A3, A4, A, B1)
        ss = s / tkf0
        s2 = ss * ss
        s4 = s2 * s2
        Fa = 1.0 + parms[0] * ss * np.arcsinh(parms[4] * ss) + (
            parms[1] - parms[2] * np.exp(-parms[3] * s2)) * s2
        Fb = 1.0 + parms[0] * ss * np.arcsinh(parms[4] * ss) + parms[5] * s4
        F = Fa / Fb
        if calcType != 'Energy':
            Fa_s = parms[0] * parms[4] * ss / np.sqrt(
                parms[4]**2 * s2 +
                1) + parms[0] * np.arcsinh(parms[4] * ss) + 2.0 * ss * (
                    parms[1] +
                    2.0 * parms[2] * parms[3] * s2 * np.exp(-parms[3] * s2) -
                    2.0 * parms[2] * np.exp(-parms[3] * s2))
      ...
      (parms[0] * np.arcsinh(parms[4] * ss) + 4.0*parms[5]*s2*ss))
        ...
        if not parms:
            parms = [(1E-8 + 0.1234) / 0.024974, 29.790, 22.417, 12.119,
                     1570.1, 55.944, 0.024974]  # a2, a4, a6, a8, a10, a12, b
        ss = s / tkf0
        s2 = ss * ss
        s4 = s2 * s2
        s6 = s4 * s2
        ...
        s10 = s4 * s6
        s12 = s6 * s6
        Fa = 1.0 + parms[0] * s2 + parms[1] * s4 + parms[2] * s6 + parms[
            3] * s8 + parms[4] * s10 + parms[5] * s12
        Fb = 1.0 + 1E-8 * s2
        F = (Fa / Fb)**parms[6]
        if calcType != 'Energy':
            dFds2 = -2 * parms[6] * F * (
                1E-8 * Fa - Fb *
                (parms[0] + 2 * parms[1] * s2 + 3 * parms[2] * s4 +
                 4 * parms[3] * s6 + 5 * parms[4] * s8 + 6 * parms[5] * s10)
            ) / (Fa * Fb)
            dFds2 /= (tkf0 * tkf0)
        ...
        F = Fa / Fb
        if calcType != 'Energy':
            dFds2 = (2.0 * parms[1] + 4.0 * parms[2] * s2) / Fb - (
                2.0 * parms[3] * Fa) / (Fb * Fb)
            dFds2 /= (tkf0 * tkf0)
        ...
        if calcType != 'Energy':
            dFds2 = 2.0 * parms[1] / Fb2 + 4 * parms[2] * s2 / (
                Fb3) + 4 * parms[3] * s4 / (Fb3 * Fb)
            dFds2 /= (tkf0 * tkf0)
        ...
        if calcType != 'Energy':
            ...
            dFds2 = (2.0 * parms[0]) / Fb - (
                2.0 * parms[1] + 4.0 * parms[2] * s2) * Fa / (Fb * Fb)
            dFds2 /= (tkf0 * tkf0)

    elif functional == 'LC94':  # \cite{hfofke} (16) # same as PW91
        if not parms:
            parms = [0.093907, 0.26608, 0.0809615, 100.0, 76.32, 0.000057767]
        ss = s / tkf0
        s2 = ss * ss
        s4 = s2 * s2
        Fa = 1.0 + parms[0] * ss * np.arcsinh(parms[4] * ss) + (
      ...
      (parms[0] * np.arcsinh(parms[4] * ss) + 4.0*parms[5]*s2*ss))
        ...
        if not parms:
            parms = [
                33873.81423879, -1044.95674204, 33885.39245476, 27374.01470582,
                3523.63738304
            ]
        ss = s / tkf0
        s2 = ss * ss
        ...
        F = Fa / Fb + 5.0 / 3.0 * s2
        if calcType != 'Energy':
            dFds2 = 2.0 * parms[1] / Fb - (parms[0] + parms[1] * s2) * (
                2 * parms[3] + 4 * parms[4] * s2) / (Fb * Fb) + 10.0 / 3.0
            dFds2 /= (tkf0 * tkf0)
...
def GGA(rho, functional='LKT', calcType='Both', split=False, **kwargs):
    ...
    if calcType == 'Energy':
        ...
        for i in range(3):
            ...
            p3.append(item.fft())
        pot3G = g[..., 0][..., None] * p3[0] + g[..., 1][
            ..., None] * p3[1] + g[..., 2][..., None] * p3[2]
