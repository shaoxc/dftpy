...
def LindhardFunction(eta, lbda, mu):
    ...
    if isinstance(eta, (np.ndarray, np.generic)):
        ...
        LindG[indx0] = 1.0 / (0.5 + 0.25 * (1.0 - eta[indx0]**2) * np.log(
            (1.0 + eta[indx0]) / np.abs(1.0 - eta[indx0])) /
                              eta[indx0]) - 3.0 * mu * eta[indx0]**2 - lbda
...
def LindhardFunction2(eta, lbda, mu):
    ...
    if isinstance(eta, (np.ndarray, np.generic)):
        ...
        LindG[cond0] = 1.0 / (0.5 + 0.25 * (1.0 - eta[cond0]**2) * np.log(
            (1.0 + eta[cond0]) / np.abs(1.0 - eta[cond0])) /
                              eta[cond0]) - 3.0 * mu * eta[cond0]**2 - lbda

        LindG[cond1] = 1.0 + eta[cond1]**2 * (1.0 / 3.0 - 3.0 * mu) - lbda
        LindG[cond2] = 2.0 - 48 * np.abs(eta[cond2] -
                                         1.0) - 3.0 * mu * eta[cond2]**2 - lbda
        #-----------------------------------------------------------------------
        # LindG += 1.6
        # cond = eta > 20.0
        # LindG[cond] = -0.6 - lbda
...
def MGPKernelOld(q, rho0, lumpfactor, maxpoints):
    ...
    indx = np.where(q != 0)
    tmpker2 = kertmp.copy()
    tmpker2[indx] = 4 * np.pi * sp.erf(q[indx])**2 * lumpfactor * np.exp(
        -q[indx]**2 * lumpfactor) / q[indx]**2 / cTF
    indx = np.where(q == 0)
