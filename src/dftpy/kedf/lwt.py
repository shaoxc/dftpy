...
def LWTPotentialEnergy(rho, etamax = 100.0, ratio = 1.1, nsp = None, delta = None, kdd = 1, \
    ...
    for i in range(nsp - 1):
        if i == 0:
            ...
            if mcalc:
                kernelDeriv0 = LWTKernelKf(q,
                                           kflists[i],
                                           KernelDeriv,
                                           etamax=etamax,
                                           out=kernelDeriv0) / kflists[i]
        ...
        if mcalc:
            kernelDeriv1 = LWTKernelKf(q,
                                       kflists[i + 1],
                                       KernelDeriv,
                                       etamax=etamax,
                                       out=kernelDeriv1) / kflists[i + 1]
            m1 = (kernelDeriv1 * rhoBetaG).ifft(force_real=True)

        mask = np.logical_and(
            kf > kflists[i], kf < kflists[i + 1] +
            1E-18)  #  1E-18 for numerical errors, must be very small
        Rmask[:] = 0.0
        Rmask[mask] = 1.0
        rhoU = rhoAlpha * Rmask
        ...
        if interp == 'newton' or interp == 'linear':
            ...
            if kdd > 1:
                if pot2G is None:
                    pot2G = kernel0 * (rhoU * t0).fft() + kernel1 * (rhoU *
                                                                     t).fft()
                else:
                    pot2G += kernel0 * (rhoU * t0).fft() + kernel1 * (rhoU *
                                                                      t).fft()
...
def LWT(rho, x=1.0,y=1.0, Sigma=0.025, interp = 'linear', kerneltype = 'WT', symmetrization = None, \
    ...
    if abs(KE_kernel_saved['rho0'] -
        ...
        if order > 1:
            ...
            KernelDerivTable = splev(
                eta, KE_kernel_saved['KernelTable'], der=1) * (-1.0 * eta)
            KE_kernel_saved['KernelDeriv'] = splrep(eta,
                                                    KernelDerivTable,
                                                    k=order)
        else:
