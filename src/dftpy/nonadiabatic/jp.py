...
def JP1Potential(rho: DirectField, j: DirectField, cutoff: float = 0, **kwargs) -> DirectField:
    ...
    sqrt_gg[0, 0, 0] = 1.0
    j_of_g = j.fft()*np.exp(-0.025*gg)
    iq_dot_j = 1j * j_of_g.dot(g)
...
def JP1(rho: DirectField, j: DirectField, calcType: List[str], **kwargs) -> Functional:
    functional = Functional(name = "JP1")
    if "V" in calcType:
