...
class Optimization(AbstractOptimization):
    ...
    def get_direction_CG(self, resA, dirA=None, method='CG-HS', **kwargs):
        ...
        if len(resA) == 1:
            ...
            beta = np.einsum('ijkl->', resA[-1] *
                             (resA[-1] - resA[-2])) / np.einsum(
                                 'ijkl->', dirA[-1] * (resA[-1] - resA[-2]))
            # print('beta', beta)
        elif method == 'CG-FR':
            beta = np.einsum('ijkl->', resA[-1]**2) / np.einsum(
                'ijkl->', resA[-2]**2)
        elif method == 'CG-PR':
            beta = np.einsum('ijkl->', resA[-1] *
                             (resA[-1] - resA[-2])) / np.einsum(
    ...
    def get_direction_LBFGS(self,
        ...
        if not lbfgs.H0:
            if len(lbfgs.s) < 1:
                ...
                gamma = np.einsum('ijkl->',
                                  lbfgs.s[-1] * lbfgs.y[-1]) / np.einsum(
                                      'ijkl->', lbfgs.y[-1] * lbfgs.y[-1])
            direction = gamma * q
        else:
    ...
    def get_direction(self,
                      ...
                      mu=None):
        if method[0:2] == 'CG':
            return self.get_direction_CG(resA,
                                         dirA=dirA,
                                         phi=phi,
                                         method=method,
                                         lbfgs=lbfgs,
            ...
            return self.get_direction_TN(resA,
                                         dirA=dirA,
                                         phi=phi,
                                         method=method,
                                         lbfgs=lbfgs,
            ...
            return self.get_direction_LBFGS(resA,
                                            dirA=dirA,
                                            phi=phi,
                                            method=method,
                                            lbfgs=lbfgs,
            ...
            return self.get_direction_DIIS(resA,
                                           dirA=dirA,
                                           phi=phi,
                                           method=method,
                                           lbfgs=lbfgs,
            ...
            raise AttributeError('The %s direction method not implemented.' %
                                 method)

    def OrthogonalNormalization(self,
                                p,
                                phi,
                                Ne=None,
                                vector='Orthogonalization'):
    ...
    def ValueAndDerivative(self,
        ...
        if algorithm == 'EMM':
            ...
            value = resN
        if vector == 'Orthogonalization':
            grad = 2.0 * np.einsum('ijkl, ijkl, ijkl->', f.potential, newphi,
                                   (p * np.cos(theta) - phi * np.sin(theta)))
        else:
    ...
    def optimize_rho(self, guess_rho=None):
        ...
        for it in range(1, self.optimization_options["maxiter"]):
            ...
            thetaDeriv0 = np.einsum('ijkl, ijkl, ijkl->', func.potential, phi,
                                    p) * 2.0
            if thetaDeriv0 > 0:
            ...
            if abs(dE) < self.optimization_options["econv"]:
                # if True :
                if len(EnergyHistory) > 2 and abs(
                        EnergyHistory[-1] - EnergyHistory[-3]
                ) < self.optimization_options["econv"]:
