...
class CBspline(object):
    ...
    def __init__(self, ions=None, grid=None, order=10, **kwargs):
        ...
        ixyzA = np.mgrid[:self.order, :self.order, :self.order].reshape(
            (3, -1))
    ...
    def Barray(self):
        if self._Barray is None:
            ...
            self._Barray = ReciprocalField(self.grid.get_reciprocal(),
                                           griddata_3d=array,
                                           rank=1)
        return self._Barray
    ...
    def _calc_PME_Qarray(self):
        ...
        for i in range(self.ions.nat):
            ...
            Mn_multi = np.einsum(
                'i, j, k -> ijk',
                self.ions.Zval[self.ions.labels[i]] * Mn[0][1:], Mn[1][1:],
                Mn[2][1:])
            l123A = np.mod(
...
class ewald(object):
    ...
    def Get_Best_eta(self, precision, gmax, ions):
        ...
        while NotGoodEta:
            ...
            upbound = 4.0 * np.pi * ions.nat * chargeSquare * np.sqrt(
                eta / np.pi) * sp.erfc(gmax / 2.0 * np.sqrt(1.0 / eta))
        ...
        for i in range(len(charges)):
            for j in range(len(charges)):
                if i != j:
                    ...
                    dij = rij.length()
                    if dij < Rcut:
                        Esum += charges[i] * charges[j] * sp.erfc(
                            np.sqrt(eta) * dij) / dij
        return Esum / 2.0
    ...
    def Eewald2(self, eta, ions, rho):
        ...
        invgg[0, 0, 0, 0] = 0.0
        gg[0, 0, 0, 0] = 0.0
        First_Sum = np.real(4.0 * np.pi * np.sum(
            strf_sq * np.exp(-gg /
                             (4.0 * eta)) * invgg)) / 2.0 / rho.grid.volume
    ...
    def Ediv2(self, precision, eta, ions, rho):
        ...
        for ix in np.arange(-N[0], N[0] + 1):
            for iy in np.arange(-N[1], N[1] + 1):
                for iz in np.arange(-N[2], N[2] + 1):
                    ...
                    R = np.einsum('j,ij->i',
                                  np.array([ix, iy, iz], dtype=np.float),
                                  rho.grid.lattice)
        ...
        for i in range(ions.nat):
            ...
            Esum += item.Zval * np.sum(charges[index] * sp.erfc(
                etaSqrt * dists[index]) / dists[index])
        Esum /= 2.0
    ...
    def Ediv1(self, ions, rho):
        ...
        TwoEhart = np.sum(
            np.real(
                HartreePotentialReciprocalSpace(density=rho) *
                np.conjugate(rhog))) / rho.grid.volume
        vloc = ions.v
        vloc[0, 0, 0, 0] = 0.0
    ...
    def Energy_real(self):
        ...
        charges = np.asarray(charges)
        for i in range(self.ions.nat):
            dists = cdist(positions, self.ions.pos[i].reshape(
                (1, 3))).reshape(-1)
            index = np.logical_and(dists < Rcut, dists > rtol)
            Esum += self.ions.Zval[self.ions.labels[i]] * np.sum(
                charges[index] * sp.erfc(etaSqrt * dists[index]) /
                dists[index])
        Esum /= 2.0
    ...
    def Energy_real_fast(self):
        ...
        for i in range(self.ions.nat):
            ...
            index = np.logical_and(dists < Rcut, dists > rtol)
            Esum += self.ions.Zval[self.ions.labels[i]] * np.sum(
                charges_local[index] * sp.erfc(etaSqrt * dists[index]) /
                dists[index])
        Esum /= 2.0
            ...
            for i0 in range(PBCmap[0, 0], PBCmap[1, 0]):
                for i1 in range(PBCmap[0, 1], PBCmap[1, 1]):
                    for i2 in range(PBCmap[0, 2], PBCmap[1, 2]):
                        ...
                        LBound = PBCpos - Rcut
                        UBound = PBCpos + Rcut
                        index1 = np.logical_and(positions > LBound,
                                                positions < UBound)
                        index1 = np.all(index1, axis=1)
    ...
    def Energy_corr(self):
        ...
        # G=0 term of local_PP - Hartree
        const = -4.0 * np.pi * (1.0 /
                                (4.0 * self.eta * self.rho.grid.volume) / 2.0)
        sum = np.float(0.0)
        for i in np.arange(self.ions.nat):
    ...
    def energy(self):
        ...
        if self.usePME:
            ...
            Ewald_Energy = self.Energy_real_fast2() + self.Energy_corr(
            ) + self.Energy_rec_PME()
            # print('fast',self.Energy_real(),self.Energy_real_fast(), self.Energy_real_fast2() )
        else:
            Ewald_Energy = self.Energy_real() + self.Energy_corr(
            ) + self.Energy_rec()
    ...
    def Forces_rec(self):
        ...
        strf = self.ions.strf(reciprocal_grid,
                              0) * self.ions.Zval[self.ions.labels[0]]
        for i in np.arange(1, self.ions.nat):
            strf += self.ions.strf(reciprocal_grid,
                                   i) * self.ions.Zval[self.ions.labels[i]]
            charges.append(self.ions.Zval[self.ions.labels[i]])
        ...
        charges = np.asarray(charges)
        for i in range(self.ions.nat):
            Ion_strf = self.ions.strf(reciprocal_grid,
                                      i) * self.ions.Zval[self.ions.labels[i]]
    ...
    def Stress_real(self):
        ...
        for ia in range(self.ions.nat):
            dists = cdist(positions, self.ions.pos[ia].reshape(
                (1, 3))).reshape(-1)
            index = np.logical_and(dists < Rcut, dists > rtol)
    ...
    def Stress_rec(self):
        ...
        for i in range(3):
            for j in range(i, 3):
                sfactor[...,
                        k] = reciprocal_grid.g[..., i] * reciprocal_grid.g[...,
                                                                           j]
                sfactor[..., k] *= 2.0 / gg[..., 0] * (1 + gg[:, :, :, 0] /
                                                       (4.0 * self.eta))
                if i == j:
                    sfactor[..., k] -= 1.0
                k += 1
        ...
        for i in range(self.ions.nat):
            sum += self.ions.Zval[self.ions.labels[i]]
        S_g0 = sum**2 * 4.0 * np.pi * (
            1.0 / (4.0 * self.eta * self.rho.grid.volume**2) / 2.0)
        k = 0
        S_rec = np.zeros((3, 3))
        for i in range(3):
            for j in range(i, 3):
    ...
    def PME_Qarray_Ewald(self):
        ...
        ## For speed
        ixyzA = np.mgrid[1:self.order + 1, 1:self.order + 1,
                         1:self.order + 1].reshape((3, -1))
        for i in range(self.ions.nat):
            Up = np.array(self.ions.pos[i].to_crys()) * nr
            Mn = []
            for j in range(3):
            ...
            l123A = np.mod(
                ...
                nr.reshape((3, 1)))
            Qarray[l123A[0], l123A[1], l123A[2]] += Mn_multi.reshape(-1)
        return DirectField(self.rho.grid,
                           griddata_3d=np.reshape(Qarray, np.shape(self.rho)),
                           rank=1)
    ...
    def Forces_rec_PME(self):
        ...
        for i in range(self.ions.nat):
            ...
            Q_derivativeA[0] = nr[0] * np.einsum(
                'i, j, k -> ijk', Mn_2[0][1:] - Mn_2[0][:-1], Mn[1][1:],
                Mn[2][1:]).reshape(-1)
            Q_derivativeA[1] = nr[1] * np.einsum('i, j, k -> ijk', Mn[0][1:],
                                                 Mn_2[1][1:] - Mn_2[1][:-1],
                                                 Mn[2][1:]).reshape(-1)
            ...
            F_rec[i] -= np.sum(
                ...
                strf[l123A[0], l123A[1], l123A[2]][:, np.newaxis],
                axis=0)
    ...
    def Stress_rec_PME_full(self):
        ...
        gg[0, 0, 0, 0] = 0.0
        Stmp = np.einsum('ijkl, ijkl->l',
                         strf_sq * np.exp(-gg / (4.0 * self.eta)) * invgg,
                         sfactor)
    ...
    def Stress_rec_PME(self):
        ...
        for i in range(3):
            for j in range(i, 3):
                ...
                if i == j:
                    sfactor[..., k] -= 1.0
                Stmp[k] = 2.0 * np.einsum(
                    'i, i->', strf_sq[mask2] * np.exp(-gg[mask2] /
                                                      (4.0 * self.eta)) *
                    invgg[mask2], sfactor[..., k][mask]).real
                k += 1
