...
def RealTimeRunnerPrintTitle(fileobj, vector_potential=False):
    sprint("{0:^17s} {1:^17s} {2:^17s} {3:^17s} {4:^17s} {5:^17s} {6:^17s} {7:^17s}".format('t', 'E', 'mu_x', 'mu_y', 'mu_z',
                                                                                    'j_x', 'j_y', 'j_z'), end='',
           fileobj=fileobj)
    if vector_potential:
...
def RealTimeRunnerPrintData(fileobj, t, E, mu, j, A=None):
...
def RealTimeRunner(config, rho0, E_v_Evaluator):
    ...
    propagate_vector_potential = config["TD"]['propagate_vector_potential']
    ...
    if mp.is_root:
        if not restart:
            with open(outfile, 'w') as fout:
                RealTimeRunnerPrintTitle(fout, vector_potential)
                RealTimeRunnerPrintData(fout, i_t0 * int_t, E, delta_mu, j_int, A_t if vector_potential else None)
        else:
            if not os.path.isfile(outfile):
                with open(outfile, 'w') as fout:
                    RealTimeRunnerPrintTitle(fout, vector_potential)

        sprint('Output data to file: {0:s}'.format(outfile))
    ...
    for i_t in range(i_t0, num_t):
        ...
        if mp.is_root:
            with open(outfile, "a") as fout:
                RealTimeRunnerPrintData(fout, (i_t + 1)*int_t, E, delta_mu, j_int, A_t if vector_potential else None)
        ...
        if (i_t + 1) % save_interval == 0 or i_t + 1 == num_t or (max_runtime > 0 and cost_t > max_runtime):
            ...
            if not os.path.isdir('./tmp') and mp.is_root:
                os.mkdir('./tmp')
            fname = './tmp/{0:s}_{1:d}.npy'.format(outfile, i_t + 1)
            if mp.size > 1:
